    ...
    fn try_from(sql: &'a Statement) -> Result<Self, Self::Error> {
        match sql {
            ...
            Statement::Query(q) => {
                ...
                let limit = limit.and_then(|v| v.to_string().parse::<u32>().ok());
    ...
    fn try_from(expr: Expression) -> Result<Self, Self::Error> {
        match *expr.0 {
            ...
            SqlExpr::Identifier(id) => {
                let column_name: String = id.value.clone().into();
                Ok(Self::Column(Arc::from(column_name)))
            }
    ...
    fn try_from(p: Projection<'a>) -> Result<Self, Self::Error> {
        match p.0 {
            ...
            SelectItem::ExprWithAlias {
                ...
                Box::new(Expr::Column(Arc::from(id.to_string().as_str()))),
                // Arc::new(alias.to_string()),
                Arc::from(alias.to_string().as_str()),
            )),
